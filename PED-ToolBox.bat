...
:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
::=======================================
:: from internet portable - downald to current directory
:m0a.x1.variableMain
cls

set mFolder=pedDownload

if not exist "%cd%\%mFolder%\." (
	mkdir "%cd%\%mFolder%"
)
set "destinationMain=%cd%"
cd pedDownload
set "destinationPD=%cd%"
...
set "fileName=%destination%\cmdMenuSel.exe"
...
:r3a.x12.2.downLoadF-taskCommand
...
set "destination=C:\ProgramData\PED-opti"
if not exist "%destination%\." mkdir "%destination%"
...
cmdMenuSel f870 "-|NEXT|- Step 1 : System Check -Update/Repair/Scan:" "-|BACK|- Main Menu" "-|MAIN MENU|- " "================= Select an option =================" "" %mm%
...
if %ERRORLEVEL% == 12 goto %menu%
if %ERRORLEVEL% == 13 start cmd /c "type C:\ProgramData\PED-opti\files\0.bootTimer.txt && echo. && cmdMenuSel f870 "Press ENTER to continue..." "
if %ERRORLEVEL% == 14 start cmd /c "type C:\ProgramData\PED-opti\files\0.bootLog.txt && echo. && cmdMenuSel f870 "Press ENTER to continue..." "
...
rem create bootTimer folders
set "destination=C:\ProgramData\PED-opti"
set "destinationF=%destination%\files"

if not exist "%destination%\." mkdir "%destination%"
if not exist "%destinationF%\." mkdir "%destinationF%"

rem Delete existings bootTimer files
if exist "%destination%\start0.2_boot1.2.bat" del "%destination%\start0.2_boot1.2.bat"
if exist "%destination%\boot1.2.ps1" del "%destination%\boot1.2.ps1"
if exist "%destinationF%\PED-BootTimer.bat" del "%destinationF%\PED-BootTimer.bat"

rem copy/install bootTimer
copy "%startFiles%" "%destinationF%"
del "%startFiles%"

timeout 2 /nobreak>nul
start %windir%\explorer.exe "%destinationF%"
if exist "%destinationF%\%nameFiles%" (
...
echo.
cmdMenuSel f870 "[ ] 1.30.Create-PED-BootTimer" "[ ] 1.31.Create-PED-BootTimerMessage"
...
set keyReg=""C:\ProgramData\PED-opti\files\%startBoot%""
set typeReg=REG_SZ
...
if %startOneClick% == 1 (
	...
	set "LinkName=PED-bootTimer"
	set "destination=C:\ProgramData\PED-opti\files"
)
set wd=%destination%
...
cmdMenuSel f870 "-|NEXT|- Accounts and Users:" "-|BACK|- Step 0 : Test and Diagnostic:" "-|MAIN MENU|- " "========== Select an option ==========" "" "[ p ] 1.Never notify" "[ ] 1.Notify me only when app try to make changes(do not dim my desktop)" "[ ] 1.Notify me only when app try to make changes(default)" "[ d ] 1.Always notify" 
...
cmdMenuSel f870 "-|NEXT|- Control Panel:" "-|BACK|- Programs" "-|MAIN MENU|- " "========== Select an option ==========" "" "[+] 1.Account Password Reset" "[+] 2.Activate Administrator account" "[+] 3.add account" "[+] 4.Local Users and Groups(Local)"
...
if %ERRORLEVEL% == 15 start cmd /c "chkdsk && echo. && cmdMenuSel f870 "Press ENTER to continue..." "
...
goto m1a.x02.6.2.speedTestCheckInternetConnection
...
robocopy "%source%" "%destination%" /e /w:5 /r:2 /COPY:DATSOU /DCOPY:DAT /MT
...
echo.
cmdMenuSel f870 "Continue without changes" "Delete" 
...
cls
echo.
cmdMenuSel f870 "Update app: %winG1%" "Type Again" "BACK"
cls
...
set "menuD2= echo."
set "menuD2=%menuD2% && echo ========== EveryDayOptimizer:"
set "menuD2=%menuD2% && echo."
...
if %ERRORLEVEL% == 13 goto %menu%
if %ERRORLEVEL% == 14 goto m1a.x5.m1a.x4.1.startUpTaskSchedulerAuto
...
:m1a.x5.m1a.x4.1.startUpTaskSchedulerAuto
if %startOneClick% == 0 (cls)
...
set menu1=m1a.x6.cleanUpJunkfiles && set menuA=Main Menu: && set menuB=Step 6 : Clean Up Junk files && set menuNextName=Step 7 : Turn on\off apps: && set menuNextGoto=m1a.x7.turnApps && set menuBackName=Step 5 : Optimizing Programs && set menuBackGoto=m1a.x5.optimizingPrograms && goto r4a.x2.CleanUp-Portable
...
if %ERRORLEVEL% == 9 goto m1a.x7.1.4.winUpdateSecurity
if %ERRORLEVEL% == 10 goto m1a.x7.1.5.WinUpdadePause
if %ERRORLEVEL% == 11 goto %menu%
...
if %ERRORLEVEL% == 6 goto m1a.x7.1.5.1.WinUpdadePause

:m1a.x7.1.5.1.WinUpdadePause
if %startOneClick% == 0 (
	cls
	echo.
...
:m1a.x7.1.5.2.WinUpdadeQualityUpdates
if %startOneClick% == 0 (
	cls
	echo.
...
:m1a.x7.1.5.3.WinUpdadeFeatureUpdates
if %startOneClick% == 0 (
	cls
	echo.
...
:m1a.x7.1.5.4.WinUpdadeDriversUpdates
cls
echo.
echo Drivers Updates:
echo.

cmdMenuSel f870 "Yes - Enable" "No - Disable"
...
::=================
	set startOneClick=1
	if %optimizeP% == 1 (
		call :m1a.x5.m1a.x4.1.startUpTaskSchedulerAuto
	) else (
...
call :m1a.x02.6.2.speedTestCheckInternetConnection
...
call :r3a.x12.3.downLoadF-cpuRam
...
cd C:\ProgramData\PED-opti\files
title	%ver%
echo 	[%ver%](2)
echo.
echo.
